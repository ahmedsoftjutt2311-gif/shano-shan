...
function Icon({name,size=20}:{name:string;size?:number}){
  ...
  const paths:any={
    ...
    x:<><path d="M5 4h3.4l4.1 5.2L16.9 4H20l-6.1 7 6.6 9h-3.4l-4.5-5.9L7.2 20H4l6.5-7.5z"/></>,
...
function shareApplication(toast:(s:string)=>void){
  const url=window.location.origin+window.location.pathname;
  const data={title:"SHANO SHAN FRAGRANCE",text:"Discover SHANO SHAN Fragrance",url};
  try{
    if((navigator as any).share){(navigator as any).share(data).catch(()=>{});return;}
    navigator.clipboard?.writeText(url).then(()=>toast("Website link copied")).catch(()=>toast(url));
  }catch{toast(url)}
}

function Header({ site }: { site: any }) {
  const { cart, user, setUser, toast } = useStore();
    ...
    <header className="header">
      <div className="nav-inner">
        ...
        <div className="nav-actions">
          ...
          <button className="icon-btn" onClick={()=>shareApplication(toast)} type="button" aria-label="Share SHANO SHAN"><Icon name="share"/></button>{canInstall && controls.show_install_button !== "0" && <button className="install-btn" onClick={install} type="button" aria-label="Install SHANO SHAN">Install</button>}
...
function Product({slug}:{slug:string}) {
  ...
  useEffect(()=>{api(`/api/products/${encodeURIComponent(slug)}`).then(x=>{const product=x.product;const firstVariant=product.variants?.[0]||null;setP(product);setSelectedVariant(firstVariant);setImg(firstVariant?.image_url||product.images?.[0]?.secure_url||"");api(`/api/products/${product.id}/reviews`).then(r=>{setReviews(r.reviews||[]);setBreakdown(r.breakdown||[])}).catch(()=>{});api(`/api/products/${product.id}/view`,{method:"POST"}).catch(()=>{})}).catch(()=>{})},[slug]);
