    ...
    <section className="section checkout">
      ...
      <div className="checkout-grid">
        <div className="panel">
          ...
          <label className="checkout-field-label">City / Town<input list="pakistan-city-list" placeholder={form.province?"Type your city...":"Select province first"} value={form.city} disabled={!form.province} onChange={e=>setForm({...form,city:e.target.value})}/><datalist id="pakistan-city-list">{(PAKISTAN_CITIES[form.province]||[]).map(city=><option value={city} key={city}/>)}</datalist></label>
...
function OrderDetail() {
  ...
  return <section className="section"><SectionHeading eyebrow="ORDER" title={o.order_number}/><div className="order-detail"><p><b>Status:</b> {o.status}</p><p><b>Payment:</b> {o.payment_status}</p><p><b>Delivery Address:</b> {o.address}, {o.city}{o.province?`, ${o.province}`:""}{o.postal_code?` ${o.postal_code}`:""}</p>{data.items.map((i:any)=><div className="sum-line" key={i.id}><span>{i.name} × {i.quantity}</span><b>{money(i.line_total)}</b></div>)}<div className="sum-line"><span>Delivery</span><b>{money(o.delivery_fee)}</b></div><div className="sum-line"><span>Total</span><b>{money(o.total)}</b></div>
    ...
    {data.payment?.method !== 'cod' && data.payment?.status !== 'Verified' && !data.payment?.receipt_url&&<div className="manual-box"><h3>Payment Receipt</h3><input type="file" accept="image/*,.pdf" onChange={e=>setFile(e.target.files?.[0]||null)}/><button onClick={async()=>{if(!file)return;const fd=new FormData();fd.append('receipt',file);try{await api(`/api/orders/${o.id}/receipt`,{method:'POST',body:fd});toast('Receipt uploaded');load()}catch(e:any){toast(e.message)}}} type="button">Upload Receipt</button></div>}
  </div></section>;
}
